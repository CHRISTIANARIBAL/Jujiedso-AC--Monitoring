...
socket.onmessage = (event) => {
    ...
    const acInfo = (window.MONITOR_ACS || []).find(item => item.name === ac);
    if (!acInfo) {
        console.warn("AC panel not found:", ac);
        return;
    }
    const logContainer = document.getElementById(`logs-${acInfo.id}`);
    if (!logContainer) {
        console.warn("Log container not found:", `logs-${acInfo.id}`);
        return;
    }
    ...
    logElement.className = "log-entry cursor-pointer border-b border-gray-100 px-4 py-2.5 hover:bg-gray-50";
    ...
    setTimeout(() => {logElement.classList.remove(
            "bg-blue-50"
        );
    }, 1500);
};
